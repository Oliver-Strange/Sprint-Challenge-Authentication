...
1. What is the purpose of using _sessions_?
   ...
   Sessions allow local memory to persist authentication information across requests to the server. Each client with have their own unique session stored on the server.

1. What does bcrypt do to help us store passwords in a secure manner.

   Bcrypt takes info passed to it and hashes over it several times with a salt so that it is no longer stored in plain text.

1. What does bcrypt do to slow down attackers?

   By hashing over the input multiple times, bcrypt slows down attackers who would need to know the times hashed, the algorithm used, and the salt to gain the decoded info.

1. What are the three parts of the JSON Web Token?

   The three parts of a JSON web token are the header(containing the algorithm and token type), the payload(containing claims for the user and any other info that is needed to be passed along), and the signature(contains the secret).
